...
hyf_getSelect(bVimNormal := false, bInput := false) {
    ...
    getTextIn32770() {
        if (StrLower(ControlGetClassNN(ControlGetFocus())) ~= "(static|button)") {
            ...
            100198
            */
            str := StrLower(ControlGetClassNN(ControlGetFocus())) ~= "static" ? ControlGetText(ControlGetFocus()) :
                WinGetText()
            str := trim(str)
            if (str ~= "^-{27}") {
...
hyf_direction(_x0, _y0, _x1, _y1) {
    ...
    getAngle(_x0, _y0, _x1, _y1) { ;NOTE _x轴正方向为0
        ...
        OutputDebug(format("i#{1} {2}:{3} {4},{5}, {6},{7} ret={8}", A_LineFile, A_LineNumber, A_ThisFunc, _x0, _y0,
            _x1, _y1, ret))
        return ret
    }
}
    ...
    SystemCursor(bShow := 0) {
        ...
        if (!c.count) {
            for id in [32512, 32513, 32514, 32515, 32516, 32642, 32643, 32644, 32645, 32646, 32648, 32649, 32650] {
                ...
                c[id] := [
                    dllcall("CreateCursor", "ptr", 0, "int", 0, "int", 0, "int", 32, "int", 32, "ptr", buffer(32 * 4,
                        0xFF), "ptr", buffer(32 * 4, 0)),
...
hyf_GetOpenedFiles(pid, fun := unset) {
    ...
    loop numberOfHandles {
        ...
        if (pid == ProcessId) {
            ...
            dllcall("DuplicateHandle", "Ptr", hProcess, "Ptr", handleValue, "Ptr", dllcall("GetCurrentProcess"), "PtrP", &
            lpTargetHandle := 0, "UInt", 0, "UInt", 0, "UInt", 2) ;DUPLICATE_SAME_ACCESS=2
            if (dllcall("GetFileType", "Ptr", lpTargetHandle) == 1) && dllcall("GetFinalPathNameByHandle", "Ptr",
                lpTargetHandle, "Str", filePath, "UInt", 512, "UInt", 0) ;;FILE_TYPE_DISK=1
...
hyf_getDocumentPath(winTitle := "") { ;获取当前窗口编辑文档的路径
    ...
    getCommandLine(winTitle := "") {
        for item in ComObjGet("winmgmts:").ExecQuery(format("Select * from Win32_Process where ProcessId={1}",
            WinGetPID(winTitle)))
    ...
    getDocPathOfHH(winTitle := "") {
        ...
        Query_Service(pobj, SID, IID := "!", bRaw := "") {
            ...
            res3 := comcall(3, psp, "Ptr", Query_Guid4String(&SID, SID), "Ptr", IID == "!" ? SID : Query_Guid4String(&
                IID, IID), "Ptr*", &pobj := 0)
        ...
        Query_Guid4String(&GUID, sz := "") {
            dllcall("ole32\CLSIDFromString", "WStr", sz ? sz : sz == "" ? "{00020400-0000-0000-C000-000000000046}" :
                "{00000000-0000-0000-C000-000000000046}", "Ptr", GUID := buffer(16, 0))
            return GUID
        }
    }
}
...
hyf_pids(exeName) {
    arr := []
    for item in ComObjGet("winmgmts:").ExecQuery(format("select ProcessId,name from Win32_Process where name='{1}'",
        exeName))
...
hyf_removeUSB(bUPan := true) { ;移除U盘
    ...
    eject(drv, bCheck := 0, bEject := 1) {
        drv := substr(drv, 1, 1)
        ;1. CreateFile
        hVolume := dllcall("CreateFile", "Str", format("\\.\{1}:", drv), "int", 0, "int", 0, "Ptr", 0, "int",
        OPEN_EXISTING := 3, "int", 0, "ptr", 0, "ptr")
        if (hVolume == -1)
            return drv . " error"
        ;2. DEVICE_NUMBER
        ...
        if dllcall("SetupAPI\CM_Request_Device_Eject", "uint", nDeviceID, "Ptr*", &nVetoType := 0, "str", nVetoType,
            "int", 1, "int", 0) {
        ...
        IOCTL_STORAGE_GET_DEVICE_NUMBER(hDevice) {
            ...
            dllcall("DeviceIoControl", "Ptr", hDevice, "uint", 0x2D1080, "int", 0, "int", 0, "ptr",
                STORAGE_DEVICE_NUMBER, "int", 12, "ptr*", 0, "ptr", 0)
...
hyf_cmd(strCode, encode := "utf-8", callback := "") { ;  GAHK32 ; Modified version : SKAN 05-Jul-2013  http://goo.gl/j8XJXY
    ...
    if !dllcall("CreateProcess", "ptr", 0, "str", strCode, "ptr", 0, "ptr", 0, "UInt", true, "UInt", 0x08000000 |
        dllcall("GetPriorityClass", "Ptr", -1, "UInt"), "ptr", 0, "ptr", 0, "ptr", STARTUPINFO, "ptr",
        PROCESS_INFORMATION) {
...
ox(winTitle := "ahk_class XLMAIN") {
    ...
    dllcall("oleacc\AccessibleObjectFromWindow", "ptr", ctlID, "uint", 0xFFFFFFF0, "ptr", IID_IDispatch, "ptr*", win :=
        ComValue(9, 0), "HRESULT")
    loop {
        try
...
hyf_props(cls, funFilter := unset) {
    ...
    for prop in cls.OwnProps() {
        ...
        OutputDebug(format("i#{1} {2}:{3} prop={4} {5}", A_LineFile, A_LineNumber, A_ThisFunc, prop, json.stringify(cls
            .%prop%, 4)))
        try
            obj[prop] := cls.%prop%
    }
...
MouseWait(tp, waitDisappear := false, ms := 5000) {
    ...
    loop {
        if (A_Cursor == tp) {
            OutputDebug(format("d#{1} {2}:{3} waitA done {4} tp={5} A_Cursor={6}", A_LineFile, A_LineNumber, A_ThisFunc,
                A_Index, tp, A_Cursor))
            break
        }
        if (A_TickCount > endtime) {
            OutputDebug(format("i#{1} {2}:{3} waitA TimeOut {4} tp={5} A_Cursor={6}", A_LineFile, A_LineNumber,
                A_ThisFunc, A_Index, tp, A_Cursor))
            return false
        }
        if (A_Cursor != tp) {
            OutputDebug(format("i#{1} {2}:{3} waitA {4} tp={5} A_Cursor={6}", A_LineFile, A_LineNumber, A_ThisFunc,
                A_Index, tp, A_Cursor))
            sleep(100)
        }
    }
    ...
    if (waitDisappear) {
        ...
        loop {
            if (A_Cursor != tp) {
                OutputDebug(format("d#{1} {2}:{3} waitB done {4} tp={5} A_Cursor={6}", A_LineFile, A_LineNumber,
                    A_ThisFunc, A_Index, tp, A_Cursor))
                break
            }
            if (A_TickCount > endtime) {
                OutputDebug(format("i#{1} {2}:{3} waitB TimeOut {4} tp={5} A_Cursor={6}", A_LineFile, A_LineNumber,
                    A_ThisFunc, A_Index, tp, A_Cursor))
                return false
            }
            if (A_Cursor = tp) {
                OutputDebug(format("i#{1} {2}:{3} waitB {4} tp={5} A_Cursor={6}", A_LineFile, A_LineNumber, A_ThisFunc,
                    A_Index, tp, A_Cursor))
                sleep(100)
            }
        }
